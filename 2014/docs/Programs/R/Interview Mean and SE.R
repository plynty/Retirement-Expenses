###############################################################################
###############################################################################
# Program Name: CE Interview Survey Sample Program (R)                        #
# Purpose: Create an interview survey expenditure table by income class       #
...
# Save the names of the vectors of file types from the interview and diary
...
library(plyr)     # load plyr package (manipulates databases easily in R)
...
sf <- paste0(mydir, "/", year, "/docs/Programs/IStub", year, ".txt")

# create a temporary file on the local disk..
tf <- tempfile()

# read the IStubYYYY.txt file into memory
...
# notes from the "Interview Mean and SE.sas" file about this section: 

  # /***************************************************************************/
  # /* STEP2: READ IN ALL NEEDED DATA                                          */
  # /* ----------------------------------------------------------------------- */
  # /* 1 READ IN THE INTERVIEW FMLY FILES & CREATE MO_SCOPE VARIABLE           */
  # /* 2 READ IN THE INTERVIEW MTAB/ITAB                                       */
...
expend.itbi <- subset( itbi , REFYR == year )
expend.mtbi <- subset( mtbi , REF_YR == year )
...
# rename the expenditure records table
expend <- expend.im
...
rm( itbi , mtbi , expend.itbi , expend.mtbi , expend.im )
...
pop.all <- dbGetQuery( db , paste( "select 10 as INCLASS, src, " , rpop.sums , "from fmly group by src" ) )
...
# the data frame 'tab' matches the final 'tab' table created by the "Interview Mean and SE.sas" example program

# this table can be viewed on the screen..
head( tab , 10 )				# view the first 10 records


# sort the columns to match the "Interview Mean and SE.lst" file #

# make a copy of the tab data frame that will be re-sorted
tab.out <- tab

tab.out <- tab.out[ , c( "title" , "estimate" , "INCLASS10" , paste0( "INCLASS" , 1:9 ) ) ]
...
tab.out[1, 3:12] <- tab.out[1, 3:12]/1000
...
write.csv( tab.out , paste0(year, " Interview Mean and SE.csv") , row.names = FALSE )
