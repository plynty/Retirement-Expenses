###############################################################################
###############################################################################
# Program Name: CE Diary Survey Sample Program (R)                            #
# Purpose: Create an diary survey expenditure table by income class           #
...
# notes from the "Diary Mean and SE.sas" file about this section: 
...
sf <- paste0(mydir, "/", year, "/docs/Programs/DStub", year, ".txt")
...
# notes from the "Diary Mean and SE.sas" file about this section: 
  ...
  # /* 2 READ IN THE DIARY EXPN/DTAB FILES                                     */
...
f.d.vars <- c( wtrep , "INCLASS" , "NEWID" , "src" )
...
expend.ed$COST <- expend.ed$COST/4
...
cus <- 
	dcast( 
		pop[ pop$src == "D"  , c( "INCLASS" , "rpop45" ) ] , 
		1 ~ INCLASS 
	)
...
# the data frame 'tab' matches the final 'tab' table created by the "Diary Mean and SE.sas" example program

# this table can be viewed on the screen..
head( tab , 10 )				# view the first 10 records


# sort the columns to match the "Diary Mean and SE.lst" file #
...
write.csv( tab.out , paste0(year, " Diary Mean and SE.csv") , row.names = FALSE )
